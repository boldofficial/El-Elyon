...
export async function POST(request: Request, {params}: {params: {id: string}}) {
    ...
    try {
        ...
        if (!resident) {
            return NextResponse.json({error: 'Resident not found for this ISP'}, {status: 404});
        }

        // Check location access
        if (!userRole.locations || !userRole.locations.includes(resident.location)) {
            await logAudit({
                clerkUserId: userId,
        ...
        return NextResponse.json({message: 'ISP published successfully'}, {status: 200});
    } catch (error: any) {
