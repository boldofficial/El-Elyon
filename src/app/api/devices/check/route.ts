...
import {getDeviceByDeviceId, checkDevice} from '@/db/queries/devices';
import {getRoleByClerkId} from '@/db/queries/roles';

/**
 * GET /api/devices/check - Check if device is registered and active
...
/**
 * POST /api/devices/check - Check if device is registered and active
 * Used for device registration checks with body payload
 */
export async function POST(request: Request) {
	try {
		...
		const result = await checkDevice(userId, deviceId);
