...
export default function SupervisorComplianceWorkspace() {
	...
	useEffect(() => {
		async function fetchData() {
			try {
				const [residentsRes, ackRes] = await Promise.all([
					fetch('/api/care/residents'),
					fetch('/api/supervisor/isp-acknowledgments'),
				]);

				setResidents(await residentsRes.json());
				setIspAcknowledgments(await ackRes.json());
			} catch (error) {
				...
				{ispAcknowledgments.length === 0 ? (
					...
					<div className="divide-y divide-gray-200">
						{ispAcknowledgments.map((item: any) => (
							<div
								key={`${item.ispId}-${item.userId || 'unassigned'}`}
								className="p-6">
								<div className="flex items-center justify-between">
		...
		<div className="space-y-6">
			<div className="text-center">
				...
				<p className="text-gray-600">
					View residents and track ISP acknowledgments
				</p>
			</div>
