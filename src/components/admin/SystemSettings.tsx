	...
	const fetchSettings = async () => {
		...
		try {
			...
			const loadedSettings = {
				complianceReminderTemplate: data.complianceReminderTemplate || '',
				guardianInviteTemplate: data.guardianInviteTemplate || '',
				alertWeekday: data.alertWeekday ?? 1,
	...
	const handleSave = async () => {
		...
		try {
			...
			const updatedSettings = {
				complianceReminderTemplate:
					updatedData.complianceReminderTemplate || '',
				guardianInviteTemplate: updatedData.guardianInviteTemplate || '',
				alertWeekday: updatedData.alertWeekday ?? 1,
		...
		<div className="space-y-6">
			...
			<div className="bg-white rounded-lg shadow-sm border p-6">
				<div className="mb-6">
					...
					<p className="text-sm text-gray-600">
						Configure email templates for automated notifications and
						invitations.
					</p>
				</div>
					...
					<div>
						...
						<textarea
							...
							onChange={(e) =>
								setSettings({
									...settings,
									complianceReminderTemplate: e.target.value,
								})
							}
						...
						<p className="text-xs text-gray-500 mt-1">
							Use variables like {'{resident_name}'}, {'{location}'},{' '}
							{'{due_date}'} for dynamic content.
						</p>
					</div>
					...
					<div>
						...
						<textarea
							...
							onChange={(e) =>
								setSettings({
									...settings,
									guardianInviteTemplate: e.target.value,
								})
							}
							className="w-full border border-gray-300 rounded-md px-3 py-2 h-32"
						...
						/>
						<p className="text-xs text-gray-500 mt-1">
							Use variables like {'{guardian_name}'}, {'{resident_name}'},{' '}
							{'{invite_link}'} for dynamic content.
						</p>
					</div>
				</div>
			</div>
			...
			<div className="bg-white rounded-lg shadow-sm border p-6">
				<div className="mb-6">
					...
					<p className="text-sm text-gray-600">
						Configure when automated compliance alerts are sent to staff and
						guardians.
					</p>
				</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
					<div>
						...
						<select
							id="alert-weekday-select"
							title="Select alert weekday"
							value={settings.alertWeekday}
							onChange={(e) =>
								setSettings({
									...settings,
									alertWeekday: parseInt(e.target.value),
								})
							}
					...
					<div>
						...
						<select
							...
							title="Select alert hour"
							value={settings.alertHour}
							onChange={(e) =>
								setSettings({...settings, alertHour: parseInt(e.target.value)})
							}
							className="w-full border border-gray-300 rounded-md px-3 py-2">
							{hours.map((hour) => (
					...
					<div>
						...
						<select
							id="alert-minute-select"
							title="Select alert minute"
							value={settings.alertMinute}
							onChange={(e) =>
								setSettings({
									...settings,
									alertMinute: parseInt(e.target.value),
								})
							}
							className="w-full border border-gray-300 rounded-md px-3 py-2">
							{minutes
				...
				<div className="space-y-4">
					<div className="flex items-center justify-between p-4 border border-gray-200 rounded-md">
						<div>
							...
							<p className="text-sm text-gray-600">
								Require staff to take a selfie when clocking in/out for identity
								verification.
							</p>
						</div>
						<label className="relative inline-flex items-center cursor-pointer">
							<input
								...
								onChange={(e) =>
									setSettings({...settings, selfieEnforced: e.target.checked})
								}
