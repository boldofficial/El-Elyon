...
export default function DatabaseCleanup() {
	...
	const handleCleanup = async () => {
		if (
			!confirm(
				'Are you sure you want to clean up orphaned user records? This action cannot be undone.'
			)
		) {
			return;
		}
		...
		try {
			...
			if (!res.ok) {
				...
				throw new Error(errorData.error || 'Failed to cleanup orphaned users');
			}
			...
			if (result.deleted.length > 0) {
				toast.success(
					`Successfully deleted ${result.deleted.length} orphaned user(s)`
				);
			} else {
		...
		<div className="bg-white rounded-lg shadow p-6">
			...
			<div className="mb-4">
				<p className="text-gray-600 mb-2">
					This tool will identify and remove user accounts that don't have
					corresponding employee records. This includes:
				</p>
				...
				<p className="text-red-600 mt-2 font-medium">
					⚠️ This action cannot be undone. Make sure you have a backup if
					needed.
				</p>
			</div>
				...
				<div className="mt-6 p-4 bg-gray-50 rounded border">
					...
					<div className="space-y-2 text-sm">
						<p>
							...
							<span className="font-medium">{results.totalUsers}</span>
						</p>
						<p>
							Orphaned users found:{' '}
							<span className="font-medium">{results.orphanedFound}</span>
						</p>
