...
export default function KioskManagement() {
	...
	const [deletingKiosk, setDeletingKiosk] = useState<string | null>(null);
	...
	const fetchKiosks = async () => {
		try {
			const response = await fetch('/api/admin/kiosks/list');
			if (response.ok) {
	...
	const fetchPairingTokens = async () => {
		try {
			const response = await fetch('/api/admin/kiosks/pairing-tokens');
			if (response.ok) {
				const data = await response.json();
				setPairingTokens(data);
			}
		} catch (error) {
			console.error('Error fetching pairing tokens:', error);
		}
	};

	const fetchLocations = async () => {
		try {
			const response = await fetch('/api/admin/employees/available-locations');
			if (response.ok) {
				const data = await response.json();
	...
	const handleCreatePairing = async (e: React.FormEvent) => {
		...
		try {
			...
			fetchPairingTokens(); // Refresh list
		} catch (error) {
	...
	const handleUpdateLabel = async (kioskId: string) => {
		try {
			const response = await fetch(`/api/admin/kiosks/${kioskId}/label`, {
				method: 'PATCH',
				headers: {'Content-Type': 'application/json'},
				body: JSON.stringify({deviceLabel: newLabel || undefined}),
			});
	...
	const handleStatusChange = async (
		...
		try {
			const response = await fetch(`/api/admin/kiosks/${kioskId}/status`, {
				method: 'PATCH',
				headers: {'Content-Type': 'application/json'},
			...
			if (!response.ok) {
				throw new Error(`Failed to ${status} kiosk`);
			}
			...
			<div className="bg-white rounded-lg shadow-sm border p-6">
				<div className="flex items-center justify-between mb-4">
					<div>
						...
						<p className="text-sm text-gray-600 mt-1">
							To set up a kiosk device, navigate to{' '}
				...
				{showPairingForm && (
					<form
						...
						<button
							className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 mt-4 md:mt-6"
							type="submit">
							Generate Token
						</button>
					</form>
				...
				{pairingData && (
					<div className="bg-blue-50 border border-blue-200 rounded p-4 mt-2">
						...
						<div className="text-xs text-gray-600 mt-2">
							Enter this token on the kiosk device to complete registration.
							<br />
							...
							{pairingData.expiresAt
								? new Date(pairingData.expiresAt).toLocaleString()
								: 'N/A'}
						</div>
					</div>
				)}
							...
							{kiosks.length === 0 ? (
								<tr>
									<td
										...
										No kiosks registered. Create a pairing token to add your
										first kiosk.
									</td>
								</tr>
							) : (
								kiosks.map((kiosk: Kiosk) => (
									<tr key={kiosk.id}>
											...
											<div>
												<div className="text-sm font-medium text-gray-900">
													{editingKiosk === kiosk.id ? (
														...
														<div className="flex items-center space-x-2">
															<span>
																{kiosk.deviceLabel || 'Unlabeled Kiosk'}
															</span>
															<button
																onClick={() => {
										...
										<td className="px-6 py-4 whitespace-nowrap">
											<div className="text-sm text-gray-900">
												{formatLastSeen(kiosk.lastSeenAt)}
											</div>
										...
										<td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
											<div className="flex space-x-2">
												{kiosk.status === 'active' && (
													<button
														onClick={() =>
															handleStatusChange(kiosk.id, 'disabled')
														}
												...
												{kiosk.status === 'disabled' && (
													<>
														<button
															onClick={() =>
																handleStatusChange(kiosk.id, 'active')
															}
														...
														<button
															onClick={() =>
																handleStatusChange(kiosk.id, 'retired')
															}
			...
			{deletingKiosk && (
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
					<div className="bg-white rounded-lg shadow-lg w-full max-w-md mx-auto p-6">
						...
						<p className="mb-4">
							Are you sure you want to permanently delete this kiosk? This
						...
						<div className="flex justify-end space-x-2">
							...
							<button
								onClick={() =>
									deletingKiosk && handleDeleteKiosk(deletingKiosk)
								}
