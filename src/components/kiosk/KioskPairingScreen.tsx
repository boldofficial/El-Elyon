...
export default function KioskPairingScreen({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const response = await fetch('/api/kiosk/complete-pairing', {
				method: 'POST',
				headers: {
			...
			const errorMessage =
				err instanceof Error ? err.message : 'Failed to pair kiosk';
			...
			<div className="max-w-md w-full bg-white p-8 rounded-lg shadow-md">
				<div className="text-center mb-8">
					...
					<p className="text-gray-600">
						Enter the pairing token provided by your administrator
					</p>
				</div>
